...
export const parseJson = async (jsonString: string): Promise<any> => {
  try {
...
export const getDocumentContents = async (
  ...
  const { stdout: documentContent } =
    extname === ".pdf"
      ? await execa("pdftotext", [filePath, "-"])
      : await execa("pandoc", [filePath]);

  return compressString(documentContent)
    .split(/\s+/)
    .slice(0, numWords)
    .join(" ");
};
