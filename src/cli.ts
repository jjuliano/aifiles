    ...
    if (template.autoOrganize) {
      try {
        ...
        if (template.folderStructure && template.folderStructure.length > 0) {
          // Template has predefined folders - LLM MUST select from these only
          if (analysis.selectedFolderPath) {
            // Validate that the selected folder actually exists in the template
            ...
            const isValidFolder = template.folderStructure.some(folder => {
              ...
              return normalizedTemplateFolder === normalizedSelectedPath ||
                     normalizedTemplateFolder.startsWith(normalizedSelectedPath + '/');
            });

            if (isValidFolder) {
              // LLM selected a valid folder from the template
              finalPath = path.join(template.basePath, analysis.selectedFolderPath, path.basename(folderName));
            } else {
              // LLM selected an invalid folder - this is an error
              console.log(`   ❌ ERROR: LLM selected invalid folder '${analysis.selectedFolderPath}' not in template structure`);
              console.log(`     Template '${template.name}' only allows these folders:`);
              template.folderStructure.forEach(folder => console.log(`       ${folder}`));

              // Use fallback to first folder
              const firstFolder = template.folderStructure[0].replace(/^\.\//, '');
              finalPath = path.join(template.basePath, firstFolder, path.basename(folderName));
              console.log(`     🔄 Using fallback folder: ${firstFolder}`);
            }
          } else {
            // No LLM selection - this is an error for templates with folderStructure
            console.log(`   ❌ ERROR: Template '${template.name}' has predefined folders but LLM provided no folder selection`);
            console.log(`     Template requires folder selection from these options:`);
            template.folderStructure.forEach(folder => console.log(`       ${folder}`));
          ...
          // Template has no predefined folders - LLM can invent its own structure
          // Use the AI's suggested path for complete creative freedom
          const aiSuggestedPath = analysis.suggestedPath || '{file_category_1}';
          const aiPromptObj = {
            ...analysis,
            file_date_created: new Date().toISOString().split('T')[0],
          };
          const suggestedFile = await replacePromptKeys(
            aiSuggestedPath + '/{file_title}',
            aiPromptObj,
            template.basePath,
            path.extname(fileName),
            template.fileNameCase || 'snake'
      ...
      if (selectedTemplate) {
        if (selectedTemplate.folderStructure && selectedTemplate.folderStructure.length > 0) {
          ...
          if (analysis.selectedFolderPath) {
            ...
            if (isValidFolder) {
              // LLM selected a valid folder from the template
              newFile = path.join(selectedTemplate.basePath, analysis.selectedFolderPath, path.basename(newFile));
            } else {
              // LLM selected an invalid folder - this is an error
              console.error(`❌ ERROR: LLM selected invalid folder '${analysis.selectedFolderPath}' not in template structure`);
              console.error(`   Template '${selectedTemplate.name}' only allows these folders:`);
              selectedTemplate.folderStructure.forEach(folder => console.error(`     ${folder}`));

              // Use fallback to first folder
              const firstFolder = selectedTemplate.folderStructure[0].replace(/^\.\//, '');
              newFile = path.join(selectedTemplate.basePath, firstFolder, path.basename(newFile));
              console.log(`   🔄 Using fallback folder: ${firstFolder}`);
            }
          } else {
            // No LLM selection - this is an error for templates with folderStructure
            console.error(`❌ ERROR: Template '${selectedTemplate.name}' has predefined folders but LLM provided no folder selection`);
            console.error(`   Template requires folder selection from these options:`);
            selectedTemplate.folderStructure.forEach(folder => console.error(`     ${folder}`));
