...
async function selectFolderFromTemplate(
  ...
  const prompt = `Based on the file analysis, select the best folder from the template's predefined structure.
...
Choose the SINGLE best folder for this file from the AVAILABLE FOLDERS listed above. Consider:
...
- Any attempt to suggest folders outside this list will be considered an ERROR

Return ONLY valid JSON with these exact fields. DO NOT include any text before or after the JSON:
{
  "selectedFolder": "folder-name",
  "selectedFolderPath": "full/path/to/folder",
  "folderConfidence": 0.85,
  "folderReasoning": "Brief explanation of why this folder is the best match"
}
...
- selectedFolder must be just the folder name (e.g., "Contracts", "Reports/Financial")
- selectedFolderPath must be the full relative path (e.g., "Contracts", "Reports/Financial")
